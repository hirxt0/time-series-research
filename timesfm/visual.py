...
def plot_gap_fill(model, df: pd.DataFrame, gap_start_dt, gap_hours=5.0, context_hours=12):
    ...
    context_len = context_hours * 60

    filled_linear = pd.Series(corrupted).interpolate(method='linear').values
    filled_ffill  = pd.Series(corrupted).ffill().values
    filled_tfm    = fill_gaps_timesfm(model, corrupted, context_len=context_len, max_gap=gap_len+10)
    filled_bidir  = bidirectional_fill(model, corrupted, context_len=context_len, max_gap=gap_len+10)
    ...
    results = {
        ...
        'TimesFM ↔':     (filled_bidir,  *metrics(filled_bidir)),
    }

    view_start = max(0, gap_start - context_len)
    view_end   = min(len(series), gap_end + context_len // 2)
